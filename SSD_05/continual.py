	...
	with Popen(["make","run"], stdout=PIPE, bufsize=1,universal_newlines=True) as p:
		...
		for line in p.stdout:
			...
			if rect:
				...
				classes.append(cla)
	print('sono le scores int8 format',scores)	
	sel=np.array(scores)>=THRESHOLD		
	print('sono rettangle all output',rettangle)	
	print('sono classes all output',classes)
	rettangle=np.array(rettangle)[sel]
	print('sono rettangle pd',rettangle)
	classes=np.array(classes)[sel]
	print('sono classes',classes)
	scores=np.array(scores)[sel]/128
	print('sono le scores',scores)
	break
